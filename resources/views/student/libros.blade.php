<x-main-layout>
    <x-slot name="title">Listado de libros</x-slot>
    ...
    <x-menuverticallibros>
        <x-slot name="categoriasDisponibles">
            @foreach ($categorias as $categoria)
                <a href="{{ route('student.categorias', ['categoria' => $categoria->id]) }}"
                    ...
                    📚
                    <span class="ml-3 truncate">{{ $categoria->name }}</span>
                </a>
            @endforeach
        </x-slot>
    ...
    <x-slot name="barraDeNavegacionInferior">
        <a href="https://www.fet.edu.co/"
            class="hover:bg-green-600 hover:text-white block rounded-md px-3 py-2 text-base font-medium transition focus:outline-none focus:ring-2 focus:ring-green-500">
            Sobre nosotros
        </a>
        <a href="https://www.fet.edu.co/"
            class="hover:bg-green-600 hover:text-white block rounded-md px-3 py-2 text-base font-medium transition focus:outline-none focus:ring-2 focus:ring-green-500">
